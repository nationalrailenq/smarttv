...
Nav.init = function(){
	...
	{
		//alert($(a).attr('id') + '::' + ($(a).attr('TABINDEX')?$(a).attr('TABINDEX'):999) + ' - ' + $(b).attr('id') + '::' + ($(b).attr('TABINDEX')?$(b).attr('TABINDEX'):999))
		return parseInt(($(a).attr('TABINDEX')?$(a).attr('TABINDEX'):999)) - parseInt(($(b).attr('TABINDEX')?$(b).attr('TABINDEX'):999));
	}
	...
	if(KeyHandler.hasSubView)
	{
		...
		this.subselected = $('.link.selected:not(.hidden)',div).attr('tabindex');
		if(!this.subselected)
		{
			this.subselected = $('.link.lastselected:not(.hidden)',div).attr('tabindex');
			if(!this.subselected)
			{
				this.subselected = $('.link.active:not(.hidden)',div).attr('tabindex');
				if(!this.subselected)
				{
					this.subselected = 0;
				}
	...
	if(viewDiv)
	{
		...
		this.selected = $('.link.selected:not(.hidden)',div).attr('tabindex');
		if(!this.selected)
		{
			this.selected = $('.link.lastselected:not(.hidden)',div).attr('tabindex');
			if(!this.selected)
			{
				this.selected = $('.link.active:not(.hidden)',div).attr('tabindex');
				if(!this.selected)
				{
					this.selected = 0;
				}
		...
		//alert($(this.links[this.selected]).attr('id') + '>>' + $(this.links[this.selected]).attr('tabindex') )
...
Nav.generic = function(keycode){
	switch(keycode)
	{
		case KeyHandler.tvKey.KEY_ENTER:
			////alert('LINK::' + $(this.links[this.selected]).attr('rel'));
			switch($(this.links[this.selected]).attr('rel'))
			{
				case 'Keypad':
